...
-(UILabel *)lTime{
    ...
    if (!_lTime) {
        _lTime = [UILabel new];
        _lTime.frame = CGRectMake(CGRectGetMaxX(_progressSr.frame)+10, 0, self.frame.size.width - CGRectGetWidth(_progressSr.frame) - 40 - CGRectGetWidth(_bCheck.frame), self.frame.size.height);
...
-(UIProgressView *)bufferPV{
    ...
    if (!_bufferPV) {
        ...
        _bufferPV.frame = CGRectMake(CGRectGetMaxX(_bCheck.frame) + 20, CGRectGetMidY(_bCheck.frame) - 2, kScreenWidth-CGRectGetMaxX(_bCheck.frame)-20*2-100, 4);
...
-(void)btnCheckSelect:(UIButton *)sender{
    ...
    if (sender.selected) {
        [_bCheck setImage:[UIImage imageNamed:@"Player"] forState:UIControlStateNormal];
    }else{
        [_bCheck setImage:[UIImage imageNamed:@"Pause"] forState:UIControlStateNormal];
    }
